...
**Verify the Vercel project is actually connected to Git before trusting "push to main, live in a minute" (found 15 August 2026).** The pattern's point 7 was silently false for MyHomework for four weeks: the Vercel project had been created by a hand-run `vercel deploy` and **never connected to the repository**, so every push deployed nothing and the live site froze on 18 July with no error anywhere. The tells in the Vercel project overview: a **"Connect Git"** button where the repo name should be, and **Source: `vercel deploy`** on the production deployment. Two companion traps from the same fix: a pnpm-workspace app needs the **Root Directory** set (`web`) or the build fails at a missing package.json, and a site whose prerender writes `<slug>.html` files needs **`cleanUrls: true`** in `vercel.json` or every page 404s at the extensionless URL its own sitemap and canonicals declare.
