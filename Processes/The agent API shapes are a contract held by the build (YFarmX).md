...
- **A new endpoint lands in four places in one commit:** the route under `src/pages/data/v1/`, the manifest (`index.json.ts`), `openapi.yaml.ts`, and new checks in `verify-agent-api.mjs`. The brief at `/connectors/muse.md` too when it changes what agents should reach for.
