...
- [[Telling a crawler-invented URL from a site bug (YFarmX)]] — junk paths shaped `<real page>/null` look exactly like a template printing a null variable, and on 15 September 2026 they were a crawler appending a segment to URLs it had already fetched. Five checks place the string outside our code, the quickest being that neither worst-hit page loads any script containing a URL assignment. Also: a 504 concentrated in one user agent is a client to block rather than an origin to tune, and which GraphQL dimensions the free plan refuses.
- [[Fitting supplied art to the 16-9 hero (YFarmX)]] — Jay's art arrives 1168x784 and the hero slot is a hard 16:9, so `object-fit: cover` silently eats 127px of height. Extend sideways, never crop; Adobe's generative expand invents garbled glyphs in the new strips on every seed, so rebuild them from texture sampled out of the image itself.
...
- [[Verify before you correct published copy]] — an agent's fact-check is evidence, not a verdict: five of sixteen "errors" in the 16 Aug 2026 sweep were wrong, and absence claims ("the figure is not in the document") are the least reliable of all. Carries the pre-publication gate (preflight script + red-team agent), the two-regime corrections rule, and the trap in machine-applying fixes.
...
- [[Article AI policy link (YFarmX)]] — exact label and bottom-of-article placement.
